// When loaded via eval(), items is already in scope; when loaded standalone, require it
if (typeof items === "undefined") {
	try {
		items = require("./items.js").items;
	} catch (e) {
		try {
			items = require("./design/items.js").items;
		} catch (e2) {}
	}
}
...
var drops = {
	...
	monsters: {
		...
		stompy: [
			...
			[1, "leather"],
			[1.0 / 500, "snring"],
		], //[0.3,"mistletoe"],[0.4,"candycane"],[1,"ornament"],[1,"ornament"],
		dknight2: [
		...
		mrpumpkin: [
			...
			[100, "candy1"],
			[10, "phelmet"],
			[10.0 / 100000, "cxjar", 1, "gcandle"],
			[10.0 / 100000, "cxjar", 1, "breyes"],
		],
		mrgreen: [
			[100, "candy0", 5],
			[100, "candy1", 10],
		...
		sparkbot: [
			[1.0 / 8000, "sparkstaff"],
			[1.0 / 200000, "platinumingot"],
			[1.0 / 25000, "platinumnugget"],
			[1.0 / 9500, "goldingot"],
			[1.0 / 1200, "goldnugget"],
...
for (var i = 0; i < cosmo2_data.length; i++) {
	var n = cosmo2_data[i][0],
		nm = cosmo2_data[i][1];
	for (var num = 0; num < nm; num++) {
		var key = "" + n + ("0" + num).slice(-2);
		if (["122", "203", "404"].indexOf(key) != -1) continue;
		drops["cosmo2"].push([1, "cx", "hairdo" + n + ("0" + num).slice(-2)]);
	}
}

drops["cosmo2"].push([1.0 / 2, "cx", "hairdo122"]);
drops["cosmo2"].push([1.0 / 3, "cx", "hairdo404"]);
drops["cosmo2"].push([1.0 / 10, "cx", "hairdo203"]);

// Populate cosmo3 - hat combinations
var cosmo3_data = [
	[1, 16],
	[2, 25],
	[3, 24],
...
for (var i = 0; i < cosmo3_data.length; i++) {
	var n = cosmo3_data[i][0],
		nm = cosmo3_data[i][1];
	for (var num = 0; num < nm; num++) {
		var key = "" + n + ("0" + num).slice(-2);
		if (["100", "114", "115", "102", "106", "113", "310", "309", "318", "400", "405"].indexOf(key) != -1) continue;
		drops["cosmo3"].push([1, "cx", "hat" + n + ("0" + num).slice(-2)]);
	}
}

drops["cosmo3"].push([1.0 / 2, "cx", "hat106"]);
drops["cosmo3"].push([1.0 / 3, "cx", "hat310"]);
drops["cosmo3"].push([1.0 / 5, "cx", "hat102"]);
drops["cosmo3"].push([1.0 / 8, "cx", "hat318"]);
drops["cosmo3"].push([1.0 / 10, "cx", "hat113"]);
drops["cosmo3"].push([1.0 / 12, "cx", "hat309"]);
drops["cosmo3"].push([1.0 / 12, "cx", "hat400"]);
drops["cosmo3"].push([1.0 / 12, "cx", "hat405"]);

// Normalize comma-separated keys (sort them)
var keys_to_fix = Object.keys(drops);
for (var i = 0; i < keys_to_fix.length; i++) {
	var key = keys_to_fix[i];
	if (key.indexOf(",") != -1) {
		var t = drops[key];
		delete drops[key];
		var parts = key.split(",");
...
for (var name in items) {
	if (!items[name]["ignore"] && !items[name]["exclusive"]) {
		if (items[name]["grades"] && ((items[name]["upgrade"] && items[name]["grades"][2] < 8) || (items[name]["compound"] && items[name]["grades"][2] < 3))) {
			...
			drops["glitch"].push([0.1, name]);
		} else if (items[name]["grades"] && items[name]["grades"][1] == 0) {
			drops["glitch"].push([0.25, name]);
		} else {
			drops["glitch"].push([1, name]);
		}
	}
}

// Populate lglitch drop table
for (var name in items) {
	if (!("g" in items[name])) {
	...
	if (!items[name]["ignore"] && !items[name]["exclusive"]) {
		if ((items[name]["grades"] && ((items[name]["upgrade"] && items[name]["grades"][2] < 8) || (items[name]["compound"] && items[name]["grades"][2] < 3))) || (items[name]["g"] || 0) > 5000000) {
			...
			drops["lglitch"].push([0.05, name]);
		} else if ((items[name]["grades"] && items[name]["grades"][1] == 0) || (items[name]["g"] || 0) > 50000) {
			drops["lglitch"].push([0.1, name]);
		} else {
			drops["lglitch"].push([1, name]);
		}
	}
}
