var skills={
	"rimeshatter":{
		"name": "Rime Shatter",
		"skin": "rimeglass",
		"type": "monster",
		"hostile": true,
		"damage_type": "magical",
		"projectile": "rimeshatter",
		"damage": 20000,
		"range": 260,
		"procs": false,
		"monster": true,
		"explanation": "Fires one large icy bolt at each of up to three contributors within 260 range. Each bolt deals 20,000 magical damage before resistance. Break the shell to stop the volley.",
		"class": []
	},
	"rimeshell":{
		"name": "Rime Shell",
		"skin": "rimeglass",
		"type": "monster",
		"class": [],
		"explanation": "At half health, the Djinn casts for 3 seconds. Deal damage equal to 5% of its maximum HP, or stun it, to break the shell and lower resistance by 160 for 5 seconds. Otherwise, it fires at up to three nearby contributors. Ordinary freezing does not interrupt it.",
		"monster": true
	},

	//Monsters
	"portal":{
		"type":"monster",
		"name":"Portal",
		"explanation":"Open a portal to different maps and instances",
	},
	"self_healing":{
		"type":"monster",
		"name":"Healing",
		"explanation":"Periodical self healing",
	},
	"healing":{
		"type":"monster",
		"name":"Healing",
		"explanation":"Heals an ally or self",
	},
	"anger":{
		"type":"monster",
		"name":"Anger",
		"explanation":"Randomly targets someone",
		"hostile":true,
	},
	"zap":{
		"type":"monster",
		"name":"Zap",
		"explanation":"Zaps you with a pure damage ray",
		"damage":1200,
		"hostile":true,
		"projectile":"sburst",
		"damage_type":"pure",
	},
	"fireball":{
		"type":"monster",
		"name":"Fireball",
		"explanation":"Hurls a fireball!",
		"damage":4000,
		"hostile":true,
		"projectile":"magic",
		"damage_type":"magical",
	},
	"frostball":{
		"type":"monster",
		"name":"Frostball",
		"explanation":"Hurls a frostball!",
		"damage":800,
		"hostile":true,
		"projectile":"frostball",
		"damage_type":"magical",
	},
	"warpstomp":{
		"type":"monster",
		"name":"Warpstomp",
		"explanation":"Warps to a random player nearby and stomps players in vicinity",
		"hostile":true,
	},
	"deepfreeze":{
		"type":"monster",
		"name":"Deepfreeze",
		"explanation":"Randomly freezes a nearby opponent",
		"damage":1200,
		"hostile":true,
		"projectile":"burst",
		"damage_type":"pure",
	},
	"multi_burn":{
		"type":"monster",
		"name":"Multi Burn",
		"explanation":"Hurl a fireball at everyone who dared attack him and burn them!",
		"ui":true,
		"hostile":true,
		"skin":"essenceoffire",
		"damage_type":"magical",
	},
	"mlight":{
		"type":"monster",
		"name":"Light",
		"explanation":"Reveals invisible entities nearby and prevents them from going invisible again for 12 seconds.",
		"ui":true,
		"hostile":true,
		"skin":"skill_light",
	},
	"dampening_aura":{
		"type":"monster",
		"name":"Dampening",
		"explanation":"Prevents teleports",
		"condition":"dampened",
		"aura":true,
		"hostile":true,
		"passive":true,
		"skin":"condition_neutral",
	},
	"weakness_aura":{
		"type":"monster",
		"name":"Weakness",
		"explanation":"Weakens",
		"condition":"weakness",
		"aura":true,
		"hostile":true,
		"passive":true,
		"skin":"condition_bad",
	},
	"curse_aura":{
		"type":"monster",
		"name":"Curse",
		"explanation":"A wretched aura",
		"condition":"cursed",
		"aura":true,
		"hostile":true,
		"passive":true,
		"skin":"condition_bad",
	},
	"multi_freeze":{
		"type":"monster",
		"name":"Multi Freeze",
		"explanation":"Hurl a frostball at everyone who dared attack him and freeze them!",
		"hostile":true,
		"ui":true,
		"skin":"essenceoffrost",
		"damage_type":"magical",
	},
	"mtangle":{
		"type":"monster",
		"name":"Tangle",
		"explanation":"Communicates through spores to activate dormant plants underneath. Slows down opponents significantly.",
		"mp":360,
		"range":480,
		"cooldown":40000,
		"duration":5000,
		"condition":"tangled",
		"target":true,
		"monsters":true,
		"hostile":true,
	},
	"stone":{
		"type":"monster",
		"name":"Stone",
		"explanation":"Looking into his eye for too long turns you to stone!",
		"condition":"stoned",
		"hostile":true,
	},
	//Players
	"drop_egg":{
		"type":"skill",
		"skin":"whiteegg",
		"name":"Drop Egg",
		"explanation":"An egg appears. Best not to ask.",
		"cooldown":2000,
		"emote":"drop_egg",
	},
	"hearts_single":{
		"type":"skill",
		"skin":"emptyheart",
		"name":"Rising Hearts",
		"explanation":"A little love goes a long way.",
		"cooldown":2000,
		"emote":"hearts_single",
	},
	"fart":{
		"type":"skill",
		"skin":"emote_fart",
		"name":"Fart",
		"explanation":"A suspicious little cloud.",
		"mp":20,
		"cooldown":2000,
		"emote":"fart",
	},
	"wiggle":{
		"type":"skill",
		"skin":"emote_wiggle",
		"name":"Wiggle",
		"explanation":"Shake it off.",
		"cooldown":2000,
		"emote":"wiggle",
	},
	"headwiggle":{
		"type":"skill",
		"skin":"emote_headwiggle",
		"name":"Head Wiggle",
		"explanation":"No. Yes. Maybe.",
		"cooldown":2000,
		"emote":"headwiggle",
	},
	"joy":{
		"type":"skill",
		"skin":"emote_joy",
		"name":"Joy",
		"explanation":"Too happy to stand still.",
		"cooldown":25000,
		"emote":"joy",
	},
	"jump":{
		"type":"skill",
		"skin":"emote_jump",
		"name":"Jump",
		"explanation":"Up you go!",
		"cooldown":4000,
		"emote":"jump",
	},
	"superjump":{
		"type":"skill",
		"skin":"emote_superjump",
		"name":"Super Jump",
		"explanation":"Way up you go!",
		"mp":10,
		"cooldown":10000,
		"emote":"superjump",
	},
	"makeawish":{
		"type":"skill",
		"skin":"emote_makeawish",
		"name":"Make a Wish",
		"explanation":"Close your eyes. This one might hear you.",
		"mp":50,
		"cooldown":240000,
		"emote":"makeawish",
	},
	"ikissyou":{
		"type":"skill",
		"skin":"emote_ikissyou",
		"name":"I Kiss You",
		"explanation":"A little kiss, from me to you.",
		"mp":0,
		"cooldown":10000,
		"range":80,
		"target":"player",
		"no_self":true,
		"emote":"ikissyou",
	},
	"highfive":{
		"type":"skill",
		"skin":"emote_highfive",
		"name":"High Five",
		"explanation":"Meet in the middle.",
		"mp":30,
		"cooldown":20000,
		"range":80,
		"target":"player",
		"no_self":true,
		"emote":"highfive",
	},
	"boop":{
		"type":"skill",
		"skin":"emote_boop",
		"name":"Boop",
		"explanation":"A tiny hello, right on the nose.",
		"mp":20,
		"cooldown":15000,
		"range":120,
		"target":"player",
		"emote":"boop",
	},
	"spotlight":{
		"type":"skill",
		"skin":"emote_spotlight",
		"name":"Spotlight",
		"explanation":"The stage knows its star.",
		"mp":150,
		"cooldown":90000,
		"range":240,
		"target":"player",
		"emote":"spotlight",
	},
	"pocketstorm":{
		"type":"skill",
		"skin":"emote_pocketstorm",
		"name":"Pocket Storm",
		"explanation":"Bring your own weather.",
		"mp":100,
		"cooldown":60000,
		"emote":"pocketstorm",
	},
	"mirrordance":{
		"type":"skill",
		"skin":"emote_mirrordance",
		"name":"Mirror Disco",
		"explanation":"One dancer. Every light.",
		"mp":250,
		"cooldown":120000,
		"emote":"mirrordance",
	},
	"snowball":{
		"type":"skill",
		"skin":"snowball",
		"consume":"snowball",
		"inventory":["snowball"],
		"name":"Snowball",
		"explanation":"Throw a snowball at someone!",
		"condition":"frozen",
		"damage":1,
		"mp":120,
		"cooldown":180,
		"range":720,
		"projectile":"snowball",
		"target":true,
		"merchant_use":true,
		"damage_type":"pure",
	},
	"scare":{
		"type":"skill",
		"skin":"skill_scare",
		"slot":[["orb","jacko"]],
		"name":"Scare",
		"explanation":"Activate your Jack-o Lantern to scare away monsters targeting you!",
		"mp":50,
		"cooldown":5000,
	},
	"power":{
		"type":"skill",
		"skin":"powerglove",
		"slot":[["gloves","powerglove"]],
		"name":"Power",
		"explanation":"Unleash the power",
		"mp":320,
		"cooldown":500,
		"condition":"power",
	},
	"xpower":{
		"type":"skill",
		"skin":"goldenpowerglove",
		"slot":[["gloves","goldenpowerglove"]],
		"name":"Power",
		"explanation":"Unleash the power",
		"mp":320,
		"cooldown":500,
		"condition":"xpower",
	},
	"warp":{
		"type":"skill",
		"skin":"warpvest",
		"slot":[["chest","warpvest"]],
		"name":"Warp",
		"explanation":"Warp space to jump into any location",
		"mp":40,
		"cooldown":200,
	},
	"temporalsurge":{
		"type":"skill",
		"skin":"orboftemporal",
		"slot":[["orb","orboftemporal"]],
		"name":"Temporal Surge",
		"explanation":"Hasten nearby spawns 15% by tapping into temporal forces!",
		"mp":1000,
		"cooldown":60*1000,
	},
	"zapperzap":{
		"type":"skill",
		"skin":"trigger",
		"slot":[["ring1","zapper"],["ring2","zapper"]],
		"name":"Zap",
		"explanation":"Zap your target for 200 damage",
		"damage":200,
		"damage_type":"magical",
		"mp":140,
		"cooldown":200,
		"range":420,
		"projectile":"gburst",
		"target":true,
		"hostile":true,
	},
	"charm":{
		"type":"skill",
		"skin":"charmer",
		"slot":[["orb","charmer"]],
		"name":"Charm",
		"explanation":"Charm a monster [%]",
		"condition":"charmed",
		"mp":40,
		"cooldown":60000,
		"target":"monster",
		"hostile":true,
		"use_range":true,
	},
	"tangle":{
		"type":"skill",
		"skin":"heartwood",
		"slot":[["mainhand","heartwood"],["offhand","heartwood"]],
		"name":"Tangle",
		"explanation":"Unleash the contained powers of nature to hinder an opponent.",
		"mp":40,
		"cooldown":60000,
		"target":true,
		"hostile":true,
		"use_range":true,
		"range_multiplier":2,
	},
	"shelter":{
		"type":"skill",
		"skin":"lanternshield",
		"slot":[["offhand","lanternshield"]],
		"name":"Shelter",
		"explanation":"Raise the lantern and shelter behind its flame.",
		"condition":"sheltered",
		"mp":240,
		"cooldown":60000,
	},
	"rspeed":{
		"type":"skill",
		"skin":"buff_speed",
		"condition":"rspeed",
		"class":["rogue"],
		"name":"Rogue Swiftness",
		"explanation":"Buff a target to increase their speed.",
		"mp":320,
		"range":320,
		"level":40,
		"cooldown":100,
		"target":"player",
	},
	"reflection":{
		"type":"skill",
		"skin":"buff_reflection",
		"condition":"reflection",
		"class":["mage"],
		"name":"Reflective Shield",
		"explanation":"Erects a magical shield around the target to grant them +20% Reflection for the duration.",
		"mp":540,
		"range":320,
		"level":60,
		"cooldown":30000,
		"target":"player",
	},
	"pickpocket":{
		"type":"skill",
		"skin":"skill_pickpocket",
		"class":["rogue"],
		"name":"Pickpocket",
		"explanation":"Try to steal a single PVP marked item from the target!",
		"range":15,
		"level":16,
		"mp":10,
		"reuse_cooldown":2*60*1000,
		"duration_min":200,
		"duration_max":2000,
		"target":"player",
		"persistent":true,
	},
	"fishing":{
		"type":"skill",
		"skin":"skill_fishing",
		"class":["merchant"],
		"wtype":["rod"],
		"name":"Fishing",
		"explanation":"Swing your rod and catch some fish!",
		"range":15,
		"level":16,
		"mp":120,
		"reuse_cooldown":48*60*1000,
		"duration_min":5000,
		"duration_max":15000,
		"persistent":true,
	},
	"mining":{
		"type":"skill",
		"skin":"skill_mining",
		"class":["merchant"],
		"wtype":["pickaxe"],
		"name":"Mining",
		"explanation":"Use your pickaxe to mine some mines!",
		"range":15,
		"level":16,
		"mp":120,
		"reuse_cooldown":124*60*1000,
		"duration_min":5000,
		"duration_max":15000,
		"persistent":true,
	},
	"mluck":{
		"type":"skill",
		"skin":"buff_luck",
		"condition":"mluck",
		"class":["merchant"],
		"name":"Merchant's Luck",
		"explanation":"Buff a target to increase their luck. 2% chance for you to receive a duplicate of their looted items!",
		"level":40,
		"mp":10,
		"range":320,
		"cooldown":100,
		"target":"player",
	},
	"mcourage":{
		"type":"skill",
		"skin":"skill_mcourage",
		"condition":"mcourage",
		"class":["merchant"],
		"name":"Merchant's Courage",
		"explanation":"When you sense danger, you know what to do...",
		"level":70,
		"mp":2400,
		"cooldown":2000,
	},
	"mfrenzy":{
		"type":"skill",
		"skin":"skill_mfrenzy",
		"condition":"mfrenzy",
		"class":["merchant"],
		"name":"Merchant's Frenzy",
		"explanation":"When you sense danger, you know what to do!",
		"level":85,
		"mp":400,
		"cooldown":20000,
	},
	"massproduction":{
		"type":"skill",
		"skin":"skill_massproduction",
		"condition":"massproduction",
		"class":["merchant"],
		"name":"Mass Production",
		"explanation":"Adds a 1-time use buff that reduces the time of your next upgrade or compound by 50%. The buff expires in 10 seconds if not used.",
		"level":30,
		"mp":20,
		"cooldown":50,
	},
	"massproductionpp":{
		"type":"skill",
		"skin":"skill_massproductionpp",
		"condition":"massproductionpp",
		"class":["merchant"],
		"name":"Mass Production++",
		"explanation":"Adds a 1-time use buff that reduces the time of your next upgrade or compound by 90%. The buff expires in 10 seconds if not used.",
		"level":60,
		"mp":200,
		"cooldown":50,
	},
	"massexchange":{
		"type":"skill",
		"skin":"skill_massexchange",
		"condition":"massexchange",
		"class":["merchant"],
		"name":"Mass Exchange",
		"explanation":"Adds a 1-time use buff that reduces the time of your next exchange by 50%. The buff expires in 10 seconds if not used.",
		"level":40,
		"mp":30,
		"cooldown":50,
	},
	"massexchangepp":{
		"type":"skill",
		"skin":"skill_massexchangepp",
		"condition":"massexchangepp",
		"class":["merchant"],
		"name":"Mass Exchange++",
		"explanation":"Adds a 1-time use buff that reduces the time of your next exchange by 90%. The buff expires in 10 seconds if not used.",
		"level":70,
		"mp":200,
		"cooldown":50,
	},
	"throw":{
		"type":"skill",
		"skin":"skill_throw",
		"class":["merchant"],
		"name":"Throw Stuff",
		"explanation":"Terrified? Just throw whatever you can find at your opponent!",
		"range":200,
		"code":"range=character.level+200",
		"positive":["essenceoflife"],
		"negative":["essenceoffire"],
		"nprop":["attack","armor"],
		"damage_type":"physical",
		"level":60,
		"mp":200,
		"cooldown":400,
		"hostile":true,
		"target":true,
		"pierces_immunity":true,
	},
	"burst":{
		"type":"skill",
		"skin":"skill_burst",
		"class":["mage"],
		"name":"Mana Burst",
		"explanation":"Converts your entire mana pool to pure damage. Deals 0.555 pure damage for each MP.",
		"mp":0,
		"ratio":0.555,
		"cooldown":6000,
		"target":true,
		"damage_type":"pure",
		"hostile":true,
		"projectile":"burst",
		"use_range":true,
	},
	"cburst":{
		"type":"skill",
		"skin":"skill_cburst",
		"class":["mage"],
		"name":"Controlled Mana Burst",
		"level":75,
		"explanation":"A skill for experienced mages. Allows you to control and channel your most powerful ability.",
		"mp":80,
		"ratio":0.5,
		"cooldown":240,
		"list":true,
		"hostile":true,
		"projectile":"burst",
		"use_range":true,
		"damage_type":"magical",
	},
	"arcane_needle":{
		"type":"skill",
		"skin":"skill_arcane_needle",
		"class":["mage"],
		"wtype":["wand"],
		"name":"Arcane Needle",
		"explanation":"Focus your wand into a needle that deals 75% damage, pierces 500 resistance, and cannot be reflected.",
		"level":90,
		"mp":160,
		"damage_multiplier":0.75,
		"rpiercing":500,
		"no_reflection":true,
		"cooldown_multiplier":1,
		"share":"attack",
		"use_range":true,
		"target":true,
		"hostile":true,
		"damage_type":"magical",
		"projectile":"arcane_needle",
		"procs":true,
		"pierces_immunity":false,
	},
	"energize":{
		"type":"skill",
		"skin":"skill_energize",
		"class":["mage"],
		"name":"Energize",
		"range":320,
		"level":20,
		"explanation":"Transfers mana to a target. As a side effect the target gains high attack speed for a short duration.",
		"cooldown":4000,
		"condition":"energized",
		"target":"player",
	},
	"light":{
		"type":"skill",
		"skin":"skill_light",
		"class":["mage"],
		"name":"Light",
		"explanation":"Reveals invisible entities nearby and prevents them from going invisible again for 12 seconds.",
		"mp":2000,
		"cooldown":0,
		"hostile":true,
	},
	"blink":{
		"type":"skill",
		"skin":"skill_blink",
		"class":["mage"],
		"name":"Blink",
		"explanation":"Teleport to a nearby location.",
		"mp":1600,
		"cooldown":1200,
	},
	"magiport":{
		"type":"skill",
		"skin":"skill_teleport",
		"class":["mage"],
		"name":"Magiport",
		"explanation":"Pull someone to your location using the magical paths that surround our world.",
		"complementary":"Unless the target doesn't have a T2+ helmet, high intelligence, or a low level, it can't resist being magiported. (Reverted [03/07/18])",
		"mp":900,
		"cooldown":0,
		"target":"player",
		"global":true,
	},
	"charge":{
		"type":"skill",
		"skin":"skill_charge",
		"class":["warrior"],
		"name":"Charge",
		"explanation":"Gain 30 Speed for a short duration.",
		"mp":0,
		"condition":"charging",
		"cooldown":40000,
	},
	"dash":{
		"type":"skill",
		"skin":"skill_dash",
		"class":["warrior"],
		"name":"Dash",
		"explanation":"Push forward, jumping over obstacles, climbing hills, defying physics!",
		"mp":120,
		"set_speed":500,
		"cooldown":0,
	},
	"taunt":{
		"type":"skill",
		"skin":"skill_taunt",
		"class":["warrior"],
		"name":"Taunt",
		"explanation":"Taunts an enemy. Steals aggro from friendly targets.", //Prevents players from escaping in pvp.
		"range":200,
		"mp":40,
		"cooldown":3000,
		"damage":1,
		"target":true,
		"hostile":true,
		"projectile":"stone",
		"pierces_immunity":true,
	},
	"agitate":{
		"type":"skill",
		"skin":"skill_agitate",
		"class":["warrior"],
		"name":"Agitate",
		"explanation":"Taunts all nearby monsters.",
		"range":320,
		"level":68,
		"mp":420,
		"cooldown":2200,
		"hostile":true,
	},
	"hardshell":{
		"type":"skill",
		"skin":"skill_hardshell",
		"class":["warrior"],
		"name":"Hard Shell",
		"explanation":"Use everything at your disposal to protect yourself from physical attacks for a short duration.",
		"condition":"hardshell",
		"level":60,
		"mp":480,
		"cooldown":16000,
	},
	"cleave":{
		"type":"skill",
		"skin":"skill_cleave",
		"class":["warrior"],
		"wtype":["axe","scythe"],
		"name":"Cleave",
		"explanation":"Swing your axe in a flurry to damage all enemies nearby!",
		"range":160,
		"level":52,
		"mp":720,
		"cooldown":1200,
		"hostile":true,
		"pierces_immunity":true,
		"damage_type":"physical",
		"procs":true,
	},
	"stomp":{
		"type":"skill",
		"skin":"skill_stomp",
		"class":["warrior"],
		"wtype":"basher",
		"name":"Stomp",
		"explanation":"Use your basher to Stomp the ground to Stun enemies nearby!",
		"condition":"stunned",
		"range":400,
		"level":52,
		"mp":120,
		"cooldown":24000,
		"duration":3200,
		"hostile":true,
		"damage_type":"physical",
	},
	"warcry":{
		"type":"skill",
		"skin":"skill_warcry",
		"class":["warrior"],
		"name":"War Cry",
		"explanation":"Motivate your allies to fight!",
		"level":70,
		"mp":320,
		"range":600,
		"cooldown":60000,
		"condition":"warcry",
	},
	"poisonarrow":{
		"type":"skill",
		"skin":"skill_poisonarrow",
		"class":["ranger"],
		"name":"Poison Arrow",
		"explanation":"Fire a single low damage but poison coated arrow at your opponent.",
		"wtype":["bow","crossbow"],
		"cooldown":300,
		"damage":200,
		"mp":360,
		"consume":"poison",
		"target":true,
		"hostile":true,
		"use_range":true,
		"projectile":"poisonarrow",
		"damage_type":"physical",
		"condition":"poisoned",
		"procs":true,
	},
	"piercingshot":{
		"type":"skill",
		"skin":"skill_piercingshot",
		"class":["ranger"],
		"name":"Piercing Shot",
		"explanation":"Fire heavy, armor piercing arrows at your opponent. They deal 75% of the damage but pierce through 500 armor.",
		"wtype":["bow","crossbow"],
		"apiercing":500,
		"damage_multiplier":0.75,
		"mp":64,
		"level":72,
		"cooldown_multiplier":1,
		"share":"attack",
		"use_range":true,
		"target":true,
		"hostile":true,
		"damage_type":"physical",
		"procs":true,
		"pierces_immunity":true,
	},
	"huntersmark":{
		"type":"skill",
		"skin":"skill_huntersmark",
		"class":["ranger"],
		"name":"Hunter's Mark",
		"explanation":"Mark an opponent for death, prevent them from stealthing away and increase damage done by 10%",
		"mp":240,
		"condition":"marked",
		"cooldown":10000,
		"target":true,
		"hostile":true,
		"range_multiplier":3,
		"range_bonus":20,
		"use_range": true
	},
	"pcoat":{
		"type":"skill",
		"skin":"skill_pcoat",
		"class":["rogue"],
		"name":"A Poisonous Touch",
		"explanation":"Coat your blade with a poison sack. Poison everyone you damage before the venom dissipates.",
		"condition":"poisonous",
		"cooldown":50000,
		"duration":7000,
		"mp":600,
		"consume":"poison",
	},
	"invis":{
		"type":"skill",
		"skin":"skill_invis",
		"class":["rogue"],
		"name":"Assassin's Smoke",
		"explanation":"Disappear into the shadows. Deal 1.25X damage when you sneak up on an enemy. Cooldown starts when you re-appear.",
		"reuse_cooldown":12000,
	},
	"mentalburst":{
		"type":"skill",
		"skin":"skill_mentalburst",
		"class":["rogue"],
		"name":"Mental Burst",
		"explanation":"Channel your mental abilities for a magical blow. If the target is defeated, the damage dealt is converted to mana.",
		"damage_type":"magical",
		"cooldown":900,
		"damage_multiplier":0.6,
		"range_multiplier":1.2,
		"range_bonus":32,
		"mp":180,
		"target":true,
		"requirements":{"int":64},
		"warning":"Highly unbalanced skill, could get nerfed or modified",
		"hostile":true,
		"projectile":"mentalburst",
		"use_range":true,
	},
	"quickpunch":{
		"type":"skill",
		"skin":"skill_quickpunch",
		"class":["rogue"],
		"name":"Quick Punch",
		"explanation":"Use your agility to quickly punch your opponent between your devastating attacks!",
		"wtype":"fist",
		"cooldown":250,
		"damage_multiplier":0.25,
		"mp":240,
		"use_range":true,
		"target":true,
		"hostile":true,
		"projectile":"quickpunch",
		"damage_type":"physical",
		"pierces_immunity":true,
		//"procs":true,
	},
	"quickstab":{
		"type":"skill",
		"skin":"skill_quickstab",
		"class":["rogue"],
		"name":"Quick Stab",
		"explanation":"Use your agility to quickly stab your opponent between your devastating attacks!",
		"wtype":"dagger",
		"cooldown":250,
		"share":"quickpunch",
		"damage_multiplier":0.36,
		"mp":320,
		"use_range":true,
		"target":true,
		"hostile":true,
		"projectile":"quickstab",
		"damage_type":"physical",
		"procs":true,
		"pierces_immunity":true,
	},
	"stack":{
		"type":"passive",
		"skin":"skill_stack",
		"class":["rogue"],
		"name":"Stacked Damage",
		"explanation":"Slowly learning the ways of your opponent each rogue attack deals one more damage.",
		"max":2000,
	},
	"shadowstrike":{
		"type":"skill",
		"skin":"skill_shadowstrike",
		"class":["rogue"],
		"name":"Shadow Strike",
		"explanation":"Use a shadow stone to strike a random enemy in a parallel universe!",
		"range":360,
		"level":70,
		"mp":320,
		"cooldown":1200,
		"consume":"shadowstone",
		"hostile":true,
		"damage_type":"physical",
	},
	"fanofknives":{
		"type":"skill",
		"skin":"fanofknives",
		"slot":[["belt","knifebelt"]],
		"class":["rogue"],
		"name":"Fan of Knives",
		"explanation":"Send five knives searching for nearby trouble. Each deals 0.85X damage.",
		"range":160,
		"level":65,
		"mp":180,
		"cooldown_multiplier":1,
		"multi":true,
		"max_targets":5,
		"share":"attack",
		"hostile":true,
		"damage_multiplier":0.85,
		"damage_type":"physical",
		"projectile":"fanofknives",
		"procs":false,
	},
	"entangle":{
		"type":"skill",
		"skin":"skill_entangle",
		"class":["mage"],
		"name":"Entangle",
		"explanation":"Unleash the contained powers of nature to hinder an opponent.",
		"level":72,
		"mp":360,
		"range":480,
		"cooldown":40000,
		"duration":5000,
		"condition":"tangled",
		"target":true,
		"monsters":true,
		"consume":"essenceofnature",
		"hostile":true,
	},
	"track":{
		"type":"skill",
		"skin":"skill_track",
		"class":["ranger"],
		"name":"Track",
		"explanation":"Use your fine-tuned senses to detect others.",
		"cooldown":1600,
		"range":1440,
		"mp":80,
	},
	"supershot":{
		"type":"skill",
		"skin":"skill_supershot",
		"class":["ranger"],
		"name":"Supershot",
		"explanation":"Deals 1.5X instant damage from an incredible distance.",
		"wtype":["bow","crossbow"],
		"damage_multiplier":1.5,
		"cooldown":30000,
		"range_multiplier":3,
		"range_bonus":20,
		"mp":400,
		"target":true,
		"hostile":true,
		"pierces_immunity":true,
		"damage_type":"physical",
		"procs":true,
		"use_range":true,
	},
	"3shot":{
		"type":"skill",
		"skin":"skill_3shot",
		"class":["ranger"],
		"name":"3-Shot",
		"explanation":"Hits 3 targets at once! Deals 0.7X damage to each target.",
		"wtype":["bow","crossbow"],
		"damage_multiplier":0.7,
		"level":60,
		"mp":200,
		"cooldown_multiplier":1,
		"multi":true,
		"share":"attack",
		"hostile":true,
		"pierces_immunity":true,
		"damage_type":"physical",
		"procs":true,
		"use_range":true,
	},
	"5shot":{
		"type":"skill",
		"skin":"skill_5shot",
		"class":["ranger"],
		"name":"5-Shot",
		"explanation":"Hits 5 targets at once! Deals 0.5X damage to each target.",
		"wtype":["bow","crossbow"],
		"damage_multiplier":0.5,
		"level":75,
		"mp":320,
		"cooldown_multiplier":1,
		"multi":true,
		"share":"attack",
		"hostile":true,
		"pierces_immunity":true,
		"damage_type":"physical",
		"procs":true,
		"use_range":true,
	},
	"4fingers":{
		"type":"skill",
		"skin":"skill_4fingers",
		"class":["ranger"],
		"name":"4 Finger Technique",
		"explanation":"Use the ancient arts to send the target to a deep meditation state. Just several taps to key chakra points does the job!",
		"level":64,
		"mp":260,
		"range":120,
		"cooldown":40000,
		"duration":5000,
		"condition":"fingered",
		"target":"player",
		"monsters":false,
		"hostile":true,
	},
	"curse":{
		"type":"skill",
		"skin":"skill_curse",
		"class":["priest"],
		"name":"Curse",
		"explanation":"Cursed opponents receive 20% more damage, deal 20% less damage and they slow down by 20.",
		"mp":400,
		"range":200,
		"duration":5000,
		"cooldown":5000,
		"condition":"cursed",
		"target":true,
		"hostile":true,
		"projectile":"curse",
		"damage_type":"magical",
	},
	"phaseout":{
		"type":"skill",
		"skin":"skill_phaseout",
		"class":["priest"],
		"name":"Phase Out",
		"explanation":"Absorb a shadow stone to phase yourself out from this universe.",
		"level":64,
		"duration":5000,
		"cooldown":4000,
		"mp":200,
		"consume":"shadowstone",
		"condition":"phasedout",
	},
	"darkblessing":{
		"type":"skill",
		"skin":"skill_dbuff",
		"class":["priest"],
		"name":"Dark Blessing",
		"explanation":"Increases damage by 25% for the duration.",
		"level":70,
		"mp":900,
		"range":600,
		"duration":8000,
		"cooldown":60000,
		"condition":"darkblessing",
	},
	"absorb":{
		"type":"skill",
		"skin":"skill_absorbsins",
		"class":["priest"],
		"name":"Absorb Sins",
		"explanation":"Pulls all targets from a friendly character.",
		"range":240,
		"cooldown":400,
		"mp":200,
		"level":55,
		"target":"player",
	},
	"partyheal":{
		"type":"skill",
		"skin":"skill_pheal",
		"class":["priest"],
		"name":"Party Heal",
		"explanation":"Heals all the party members.",
		"output":400,
		"action":"heal",
		"levels":[
			[0,400],
			[60,600],
			[72,720],
			[80,800],
		],
		"mp":400,
		"cooldown":200,
		"party":true,
		"projectile":"partyheal",
		"damage_type":"magical",
		"heal":true,
		"multi":true,
	},
	"selfheal":{
		"type":"skill",
		"skin":"skill_selfheal",
		"class":["paladin"],
		"name":"Heal",
		"explanation":"Heal yourself",
		"output":400,
		"action":"heal",
		"levels":[
			[0,400],
			[60,600],
			[72,720],
			[80,800],
		],
		"mp":20,
		"cooldown":1200,
		"projectile":"partyheal",
		"damage_type":"magical",
		"heal":true,
	},
	"mshield":{
		"type":"skill",
		"skin":"skill_mshield",
		"class":["paladin"],
		"name":"Mana Shield",
		"explanation":"Create a magical shield around yourself to absorb damage [Toggle]",
		"condition":"mshield",
		"toggle":true,
		"mp":0,
		"duration":999999999,
		"cooldown":500,
		"exclusive_condition":"aether_shield",
	},
	"aether_shield":{
		"type":"skill",
		"skin":"skill_aether_shield",
		"class":["paladin"],
		"name":"Aether Shield",
		"explanation":"Magical damage still wounds you, but restores MP from the HP lost. Replaces Mana Shield. [Toggle]",
		"level":60,
		"mp":0,
		"share":"mshield",
		"condition":"aether_shield",
		"exclusive_condition":"mshield",
		"toggle":true,
		"duration":999999999,
		"mp_return_levels":[[60,0.5],[70,0.6],[80,0.7],[90,0.8],[100,0.9],[110,1]],
	},
	"cleansing_light":{
		"type":"skill",
		"skin":"skill_cleansing_light",
		"class":["paladin"],
		"name":"Cleansing Light",
		"explanation":"Lift every harmful combat condition from an ally.",
		"level":30,
		"mp":320,
		"cooldown":24000,
		"range":240,
		"fixed_range":true,
		"target":"player",
		"no_self":true,
	},
	"guardians_oath":{
		"type":"skill",
		"skin":"skill_guardians_oath",
		"class":["paladin"],
		"name":"Guardian's Oath",
		"explanation":"Take 35% of an ally's damage for 8 seconds. The HP you lose restores MP, whatever the damage type.",
		"level":50,
		"mp":320,
		"cooldown":24000,
		"duration":8000,
		"range":240,
		"fixed_range":true,
		"link_range":360,
		"ratio":0.35,
		"mp_return_levels":[[50,0.4],[60,0.5],[70,0.6],[80,0.7],[90,0.8],[100,0.9],[110,1]],
		"condition":"guardians_oath",
		"target":"player",
		"no_self":true,
	},
	"beacon_of_resolve":{
		"type":"skill",
		"skin":"skill_beacon_of_resolve",
		"class":["paladin"],
		"name":"Beacon of Resolve",
		"explanation":"Hold the line! Nearby friendly players gain 15 Fortitude and one of each Courage for 8 seconds.",
		"level":70,
		"mp":640,
		"cooldown":60000,
		"duration":8000,
		"range":480,
		"condition":"beacon_of_resolve",
	},
	"paladin_aura":{
		"type":"skill",
		"skin":"paladin_aura_bulwark",
		"class":["paladin"],
		"name":"Paladin Aura",
		"explanation":"Carry one aura that strengthens nearby friendlies. Use the skill to change its form.",
		"level":60,
		"mp":0,
		"cooldown":500,
		"range":320,
		"aura":true,
		"default_state":"bulwark",
		"rank_levels":[60,70,80,90,100,110],
		"states":{
			"bulwark":{
				"name":"Aura of the Bulwark",
				"condition":"paladin_aura_bulwark",
				"values":{"armor":[30,40,50,60,100,110],"hp":[500,600,700,800,1500,1750]},
			},
			"sanctuary":{
				"name":"Aura of Sanctuary",
				"condition":"paladin_aura_sanctuary",
				"values":{"resistance":[30,40,50,60,100,110],"mp":[150,200,250,300,600,700]},
			},
			"zeal":{
				"name":"Aura of Zeal",
				"condition":"paladin_aura_zeal",
				"values":{"output":[1,1,2,2,4,5],"frequency":[1,2,2,3,4,4]},
			},
			"warding":{
				"name":"Aura of Warding",
				"condition":"paladin_aura_warding",
				"values":{
					"firesistance":[6,7,8,10,15,18],
					"fzresistance":[6,7,8,10,15,18],
					"phresistance":[6,7,8,10,15,18],
					"pnresistance":[6,7,8,10,15,18],
					"stresistance":[6,7,8,10,15,18],
					"mp_reduction":[1,2,2,3,5,6]
				},
			},
		},
	},
	"purify":{
		"type":"skill",
		"skin":"skill_purify",
		"class":["paladin"],
		"name":"Purify",
		"level":60,
		"damage":2000,
		"damage_type":"pure",
		"explanation":"Deals 2000 pure damage purifying the target, removes all buffs and debuffs dealing additional 400 damage for each and receive the Purifier buff if the target is defeated!",
		"kill_buff":"purifier",
		"projectile":"purify",
		"hostile":true,
		"target":true,
		"mp":360,
		"cooldown":24000,
		"range":480,
	},
	"shield_slam":{
		"type":"skill",
		"skin":"skill_shield_slam",
		"class":["paladin"],
		"name":"Shield Slam",
		"explanation":"Drive your shield into an enemy. Deals physical damage equal to 3 times Attack plus 12 times Armor, counting up to 1,000 Armor. Cannot critically strike or trigger item effects.",
		"level":60,
		"offhand_type":"shield",
		"cooldown":600,
		"mp":2000,
		"damage_multiplier":3,
		"armor_multiplier":12,
		"armor_cap":1000,
		"use_range":true,
		"target":true,
		"hostile":true,
		"projectile":"shield_slam",
		"damage_type":"physical",
		"procs":false,
		"pierces_immunity":true,
	},
	"smash":{
		"type":"skill",
		"skin":"skill_smash",
		"class":["paladin"],
		"name":"Smash",
		"explanation":"Smash your opponent using your mace!",
		"wtype":"mace",
		"cooldown":320,
		"level":10,
		"damage_multiplier":0.36,
		"mp":380,
		"use_range":true,
		"target":true,
		"hostile":true,
		"projectile":"smash",
		"damage_type":"physical",
		"procs":true,
		"pierces_immunity":true,
	},
	"alchemy":{
		"type":"skill",
		"skin":"skill_alchemy",
		"class":["mage"],
		"name":"Alchemy",
		"explanation":"Dangerous skill, use with caution! Converts your firstfound item into pure gold!",
		"variance":0.2,
		"output":0.8,
		"action":"rate",
		"levels":[
			[40,0.8],
			[50,0.86],
			[60,0.92],
			[70,1.0],
			[80,1.06],
			[90,1.10],
			[100,1.12],
		],
		"level":40,
		"mp":347,
		"cooldown":8000,
		"party":true,
	},
	"revive":{
		"type":"skill",
		"skin":"skill_revive",
		"class":["priest"],
		"name":"Revive!",
		"explanation":"If a fallen comrade's gravestone has been healed fully, revives them using an Essence of Life!",
		"mp":500,
		"cooldown":200,
		"range":240,
		"consume":"essenceoflife",
		"target":"player",
	},
	"regen_hp":{
		"type":"ability",
		"skin":"regen_hp",
		"name":"Regenerate HP",
		"cooldown_group":"potion",
		"share":"use_hp",
		"cooldown_multiplier":2,
		"explanation":"Restore 50 HP without a potion. Shares a cooldown with MP regeneration and HP/MP potions. Using this ability makes all four unavailable for 4 seconds.",
	},
	"regen_mp":{
		"type":"ability",
		"skin":"regen_mp",
		"name":"Regenerate MP",
		"cooldown_group":"potion",
		"share":"use_mp",
		"cooldown_multiplier":2,
		"explanation":"Restore 100 MP without a potion. Shares a cooldown with HP regeneration and HP/MP potions. Using this ability makes all four unavailable for 4 seconds.",
	},
	"use_mp":{
		"type":"ability",
		"skin":"use_mp",
		"name":"Use MP Potion",
		"cooldown_group":"potion",
		"explanation":"Use the last MP potion in your inventory. Shares a cooldown with HP potions and HP/MP regeneration. Ordinary potions make all four unavailable for 2 seconds. Without a potion, restore 100 MP and wait 4 seconds instead.",
		"cooldown":2000,
	},
	"use_hp":{
		"type":"ability",
		"skin":"use_hp",
		"name":"Use HP Potion",
		"cooldown_group":"potion",
		"explanation":"Use the last HP potion in your inventory. Shares a cooldown with MP potions and HP/MP regeneration. Ordinary potions make all four unavailable for 2 seconds. Without a potion, restore 50 HP and wait 4 seconds instead.",
		"cooldown":2000,
	},
	"use_town":{
		"type":"ability",
		"skin":"use_town",
		"name":"Town",
		"explanation":"Teleports you to the center of the map.",
		"cooldown":0,
	},
	"attack":{
		"type":"ability",
		"skin":"attack",
		"name":"Attack",
		"explanation":"Attack the target",
		"target":true,
		"hostile":true,
		"pierces_immunity": true,
		"use_range":true,
		"projectile":"momentum",
		"procs":true,
	},
	"heal":{
		"type":"ability",
		"skin":"heal",
		"name":"Heal",
		"class":["priest"],
		"explanation":"Heal the target",
		"cooldown_multiplier":1,
		"share":"attack",
		"target":true,
		"use_range":true,
		"pierces_immunity": true,
		"projectile":"plight",
		"damage_type":"magical",
		"heal":true,
		"procs":true,
	},
	"travel":{
		"type":"ability",
		"skin":"travel",
		"name":"Travel!",
		"explanation":"Where would you like to visit?",
	},
	"stop":{
		"type":"ability",
		"skin":"stop_g",
		"name":"Stop",
		"explanation":"Stops your character. Cancels all channeling abilities and active skills.",
		"cooldown":0,
	},
	"snippet":{
		"type":"utility",
		"skin":"run_snippet1",
		"name":"Snippet",
		"explanation":"Maps a code snippet to a keypress.",
		"skins":["run_snippet0","run_snippet1","run_snippet2"],
		"code":true,
	},
	"emotion":{
		"type":"utility",
		"skin":"run_emotion",
		"name":"Random Emote",
		"explanation":"Show a random Emote you've unlocked!",
	},
	"pure_eval":{
		"type":"utility",
		"skin":"run_eval0",
		"name":"Pure Eval",
		"explanation":"[Advanced] Maps a code snippet to be run directly inside the game, rather than using CODE. (Not to be confused with the text command, /eval, which evals inside the Code iframe)",
		"skins":["run_eval0","run_eval1","run_eval2"],
		"code":true,
	},
	"esc":{
		"type":"utility",
		"skin":"esc",
		"name":"ESC",
		"explanation":"Calls the esc_pressed() function inside the game. Major functionality.",
		"ui":false,
	},
	"move_up":{
		"type":"utility",
		"skin":"up",
		"name":"Move Up",
		"explanation":"Moves the character upwards",
		"ui":false,
	},
	"move_down":{
		"type":"utility",
		"skin":"down",
		"name":"Move Down",
		"explanation":"Moves the character downwards",
		"ui":false,
	},
	"move_left":{
		"type":"utility",
		"skin":"left",
		"name":"Move Left",
		"explanation":"Moves the character leftwards",
		"ui":false,
	},
	"move_right":{
		"type":"utility",
		"skin":"right",
		"name":"Move Right",
		"explanation":"Moves the character rightwards",
		"ui":false,
	},
	"toggle_code":{
		"skin":"code",
		"name":"Toggle Code",
		"ui":false,
	},
	"toggle_run_code":{
		"skin":"run_code",
		"name":"Engage/Disengage Code",
		"ui":false,
	},
	"toggle_inventory":{
		"type":"utility",
		"skin":"inventory",
		"name":"Toggle Inventory",
		"explanation":"Toggles the inventory",
		"ui":false,
	},
	"toggle_stats":{
		"type":"utility",
		"skin":"stats",
		"name":"Toggle Stats",
		"explanation":"Toggles the character sheet",
		"ui":false,
	},
	"toggle_character":{
		"type":"utility",
		"skin":"character",
		"name":"Toggle Character",
		"explanation":"Toggles the character focus",
		"ui":false,
	},
	"open_snippet":{
		"type":"utility",
		"skin":"snippet",
		"name":"Open Snippet",
		"explanation":"Open the Code Snippet interface",
		"ui":false,
	},
	"interact":{
		"type":"utility",
		"skin":"interact",
		"name":"Interact",
		"explanation":"Interact with the nearest game entity, a door, doorway, or NPC.",
		"ui":false,
	},
	"gm":{
		"type":"gm",
		"skin":"gm",
		"name":"GM Abilities",
		"explanation":"Jump, Jail, Mute, Use Any Skill",
	}
};
for(var name in skills){
	if("condition" in skills[name] && !("duration" in skills[name]) && skills[name]["condition"] in conditions && conditions[skills[name]["condition"]]){
		try{
			skills[name]["duration"]=conditions[skills[name]["condition"]]["duration"];
		}catch(e){
			console.error(conditions[skills[name]["condition"]]+" has no duration");
		}
	}
}
