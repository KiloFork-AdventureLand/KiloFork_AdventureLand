...
function runtime(invoke, image_mode = "success") {
	const log = [], blobs = new Map(), revoked = [];
	...
	const context = vm.createContext({
		...
		URL: ImageURL, Image: TestImage, Blob, TextDecoder, Uint8Array, DataView, ArrayBuffer,
		location: { href: "https://adventure.land/", origin: "https://adventure.land", protocol: "https:", reload() { context.reloads++; } },
		navigator: { languages: ["tr-TR"] }, phrase, reloads: 0,
...
function loader_for(desktop) {
	let middleware;
	const loader = { pre: fn => { middleware = fn; }, onComplete: { once() {} } };
	desktop.loadImages(loader);
	return url => new Promise(resolve => {
		let completions = 0, next_calls = 0;
		const resource = { url, name: url, complete() { completions++; } };
		middleware(resource, () => { next_calls++; resolve({ resource, completions, next_calls }); });
	});
}
...
test("PIXI gets local bytes under its original texture key; new versions and foreign origins use the network", async () => {
	let calls = 0;
	const { desktop, blobs } = runtime(() => { calls++; return Promise.resolve(bundle); });
	...
	for (const url of [image_url + "-new", image_url.split("?")[0], "/images/new.png?v=1", "https://example.org" + image_url]) {
...
test("unsupported, corrupt, rejected and stalled cache reads all fall back without retrying", async () => {
	for (const invoke of [
		...
		let calls = 0;
		const { desktop } = runtime(() => { calls++; return invoke(); });
		const load = loader_for(desktop);
		for (let i = 0; i < 2; i++) {
...
test("headless mode never touches PIXI or starts a cache read", () => {
	...
	Object.defineProperty(context, "PIXI", { get() { assert.fail("Headless PIXI access"); } });
	desktop.loadImages(new Proxy({}, { get() { assert.fail("Headless loader access"); } }));
});
...
test("language uses system fallback input and ignores a late result after a player choice", async () => {
	...
	const { desktop } = runtime((command, args) => {
		...
		return new Promise(resolve => { initial = resolve; });
	});
...
test("account and explicit choices sync to native; only a guest can auto-reload, once", async () => {
	for (const [account, initialized, source, expected] of [
		[true, true, "account", "en"], [false, true, "account", "en"], [false, true, "cookie", "en"], [false, false, "browser", null],
	]) {
		const { context } = runtime((command, args) => { assert.equal(args.preferred, expected); return args.preferred || "de"; });
...
test("all real desktop catalogs are shipped, separately from the image cache", () => {
	...
	assert.deepEqual(locales.map(entry => entry.code), phrase.languages.map(entry => entry.code));
...
test("disconnect polling times out without overlapping requests or restarting after a late response", async () => {
	...
	let calls = 0, respond;
	context.api_call = () => { calls++; return new Promise(resolve => { respond = resolve; }); };
