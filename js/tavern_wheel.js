...
function wheel_start(data) {
	...
	var TAU = Math.PI * 2,
		...
		current = tavern_wheel.rotation % TAU,
		target = -((data.index + 0.5) * step + jitter),
		delta = (((target - current) % TAU) + TAU) % TAU,
...
function wheel_finish(data) {
	...
	$(".wheelhint").html(data.won ? "<span class='gold'>+" + to_pretty_num(data.net) + "</span>" : "<span style='color: #E05A4A'>-" + to_pretty_num(data.gold) + "</span>");
	wheel_sound(data.won ? "coins" : "drop");
	if (data.won && data.net >= 10000000) wheel_sound("level_up");
	setTimeout(function () {
...
function wheel_draw(rotation, now) {
	...
	if (!tavern_wheel.image) tavern_wheel.image = ctx.createImageData(W, H);
	var image = tavern_wheel.image,
		px = image.data,
		rim = [109, 61, 75],
		wood = [179, 117, 75],
		wood2 = [157, 95, 62],
	...
	// Which slice sits under the pointer; a change flicks the needle and ticks while spinning.
	...
	var blink = tavern_wheel.blink,
		lit = blink && floor((now - blink.start) / 150) % 2 == 0 ? blink.index : -1;
	for (var y = cy - R - 2; y <= cy + R + 2; y++) {
		for (var x = cx - R - 2; x <= cx + R + 2; x++) {
			...
			// Three white speckles per slice, fixed to the wheel so they turn with it.
			if (index != lit) {
	...
	var top = cy - R,
		flick = now - tavern_wheel.kick < 90 ? -1 : 0;
	for (var x = cx - 2; x <= cx + 2; x++) for (var y = top - 5; y <= top - 3; y++) put(x, y, rim);
	put(cx, top - 4, wood2);
	for (var y = top - 3; y <= top + 5; y++) put(cx + (y > top + 1 ? flick : 0), y, black);
